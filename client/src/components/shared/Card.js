...
// Ultra-optimized Logo component
const Logo = memo(
  ({ src, alt, isActive, onClick }) => (
    ...
    >
      <Image
        ...
        width={28}
        height={28}
        className="w-7 h-7 object-cover rounded-lg hover:scale-105 active:scale-95"
        loading="eager" // Always eager load small logos
      />
    </button>
  ),
...
// Ultra-optimized Toast Handler
const useToastHandler = (id) => {
  const toastRef = useRef(id);

  return useCallback((status, message) => {
    switch (status) {
      case "loading":
...
// Ultra-optimized Favorite Button
const FavoriteButton = memo(({ product, favorite }) => {
  const [addToFavorite, { isLoading: isAdding }] = useAddToFavoriteMutation();
  const [removeFromFavorite, { isLoading: isRemoving }] =
    useRemoveFromFavoriteMutation();
  const isLoading = isAdding || isRemoving;
  const showToast = useToastHandler("favorite-toast");
  ...
  const handleClick = useCallback(
    (e) => {
      ...
      if (favorite) {
        ...
        removeFromFavorite({ id: favorite?._id })
          .unwrap()
          .then((data) => showToast("success", data?.description || "Removed"))
          .catch(() => showToast("error", "Failed to remove"));
      } else {
        showToast("loading", "Adding...");
        addToFavorite({ product: product?._id })
          .unwrap()
          .then((data) => showToast("success", data?.description || "Added"))
  ...
  return (
    <button
      className="bg-white/90 p-2 rounded-full shadow-sm border border-gray-100 hover:scale-105 active:scale-95"
      onClick={handleClick}
      type="button"
...
// Ultra-optimized Campaign Badge
const CampaignBadge = memo(
  ({ campaign }) => {
    if (!campaign) return null;
    ...
    return (
      <span className="absolute bottom-3 right-3 z-20 text-xs bg-white/90 px-3 py-1.5 rounded-full shadow-sm border border-gray-100 inline-flex items-center">
        <span className="flex flex-row gap-x-1.5 items-center font-medium">
          {content}
        </span>
      </span>
  ...
  (prev, next) => {
    if (!prev.campaign && !next.campaign) return true;
    if (!prev.campaign || !next.campaign) return false;
    return (
      prev.campaign.state === next.campaign.state &&
...
// Ultra-optimized Product Details
const ProductDetails = memo(
  ({ product, handleProductClick }) => {
    // Pre-compute values
    const hasColors = product?.variations?.colors?.length > 0;
    const hasSizes = product?.variations?.sizes?.length > 0;
    const colorCount = hasColors ? product.variations.colors.length : 0;
    const sizeCount = hasSizes ? product.variations.sizes.length : 0;
    ...
    return (
      <div
        className="flex flex-col gap-y-3 px-3 sm:px-4 py-3 h-full relative cursor-pointer"
        onClick={handleProductClick}
      >
        {/* Variations */}
        {(hasColors || hasSizes) && (
          <div className="flex flex-row items-center flex-wrap gap-2">
            {hasColors && (
              <Badge className="text-indigo-800 bg-indigo-50 border border-indigo-100">
                {colorCount} Colors
              </Badge>
            )}

            {hasColors && hasSizes && (
              <div className="h-4 border-l w-[1px] border-gray-200 hidden sm:block"></div>
            )}

            {hasSizes && (
              <Badge className="text-purple-800 bg-purple-50 border border-purple-100">
                {sizeCount} Sizes
              </Badge>
            )}
          </div>
        ...
        {/* Title */}
        <h2 className="line-clamp-1 text-sm sm:text-base font-medium hover:text-blue-500">
          {title}
        </h2>
        ...
        <div className="flex flex-row items-center justify-between mt-auto">
          {/* Price */}
          <span className="flex items-center border-2 border-emerald-500 rounded-full py-1 px-2.5 text-xs sm:text-sm font-medium bg-emerald-50/80">
            <span className="text-emerald-600 !leading-none">${price}.00</span>
          </span>

          {/* Rating */}
          <span className="flex flex-row items-center gap-x-1 bg-amber-50/80 px-2 py-1 rounded-full">
            <AiFillStar className="text-amber-400 w-3.5 h-3.5" />
            <span className="text-xs font-medium text-amber-700">
              {reviewCount}
            </span>
...
// Main Card component - ultra-optimized for instant loading
const Card = ({ index = 0, product, ...rest }) => {
  // Core state management - minimized
  const router = useRouter();
  const user = useSelector((state) => state?.auth?.user);
  ...
  // Set active image with fallback - runs only once
  useEffect(() => {
    setActiveImageSrc(product?.thumbnail?.url || "/placeholder.svg");
  }, [product?.thumbnail?.url]);

  // Ultra-optimized image switching
  const handleImageSwitch = useCallback(
    (e, newSrc) => {
      e.stopPropagation();
      if (activeImageSrc === newSrc) return;
      setActiveImageSrc(newSrc);
    },
    [activeImageSrc]
  );

  // Ultra-optimized navigation
  const handleProductClick = useCallback(() => {
    router.push(
      `/product?product_id=${product?._id}&product_title=${product?.title
        .replace(/ /g, "-")
  ...
  const showResetButton =
    activeImageSrc !== product?.thumbnail?.url && activeImageSrc !== null;
  const hasCampaign = Boolean(product?.campaign);
  const brandLogoUrl = product?.brand?.logo?.url;
  const brandLogoId = product?.brand?.logo?.public_id;
  const storeLogoUrl = product?.store?.thumbnail?.url;
  const storeLogoId = product?.store?.thumbnail?.public_id;
  const thumbnailUrl = product?.thumbnail?.url;
  const thumbnailId = product?.thumbnail?.public_id;

  // Main render with ultra-optimizations
  return (
    <div
      {...rest}
      className={`
        flex-shrink-0 flex flex-col bg-white rounded-xl overflow-hidden relative w-full
      ...
      style={{
        maxWidth: "100%",
        opacity: 0,
        animation: `cardFadeIn 0.3s ${Math.min(index * 0.03, 0.15)}s forwards`,
      }}
    >
      ...
      <div className="relative w-full pt-[75%]">
        ...
        <div className="absolute inset-0">
          {/* Main product image - always eager load for instant display */}
          <Image
            src={activeImageSrc || "/placeholder.svg"}
            alt={thumbnailId || "Product image"}
            fill
            sizes="(max-width: 640px) 100vw, (max-width: 768px) 50vw, (max-width: 1024px) 33vw, 25vw"
        ...
        {hasLogo && (
          <div className="flex flex-row gap-x-2 absolute top-3 left-3 z-20">
            {brandLogoUrl && (
              <Logo
                src={brandLogoUrl}
                alt={brandLogoId || "Brand logo"}
                onClick={(e) => handleImageSwitch(e, brandLogoUrl)}
                isActive={activeImageSrc === brandLogoUrl}
              />
            )}
            {storeLogoUrl && (
              <Logo
                src={storeLogoUrl}
                alt={storeLogoId || "Store logo"}
                onClick={(e) => handleImageSwitch(e, storeLogoUrl)}
                isActive={activeImageSrc === storeLogoUrl}
              />
            )}
          </div>
        )}
        ...
        {showResetButton && (
          <button
            onClick={(e) => handleImageSwitch(e, thumbnailUrl)}
            className="absolute top-3 right-3 z-20 bg-white/90 p-2 rounded-full shadow-sm border border-gray-100 hover:scale-105 active:scale-95"
            type="button"
            aria-label="View main product image"
          >
            <IoReturnUpBack className="w-3.5 h-3.5 text-gray-700" />
        ...
        {/* Favorite Button */}
        <div className="absolute bottom-3 left-3 z-20">
          <FavoriteButton product={product} favorite={favorite} />
        </div>
        {/* Simple overlay */}
      ...
      {/* CSS Animations - minimal and optimized */}
      <style jsx>{`
