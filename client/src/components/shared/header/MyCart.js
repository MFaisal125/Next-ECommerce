    ...
    <div className="relative">
      ...
      {isOpen && (
        <OutsideClick
          ...
          className="absolute top-full right-0 mt-2 w-[280px] sm:w-[350px] max-w-[calc(100vw-2rem)] bg-white border border-gray-200 rounded-xl shadow-lg p-3 flex flex-col z-50"
        >
                ...
                <div className="flex flex-col gap-y-2">
                  <div className="max-h-[280px] overflow-y-auto pr-1 custom-scrollbar">
                    {user?.cart?.map(({ product, quantity, _id }) => (
                      <div
                        key={product?._id}
                        className="flex flex-row gap-x-2 transition-all border border-transparent p-2 rounded hover:border-gray-200 group relative"
                      >
                        <div className="relative h-[50px] w-[50px] flex-shrink-0 overflow-hidden rounded">
                          <Image
                            ...
                            alt={
                              product?.thumbnail?.public_id || "Product image"
                            }
                            width={50}
                            height={50}
                            className="rounded h-full w-full object-cover transition-transform group-hover:scale-105"
                            onError={(e) => {
                              e.currentTarget.src =
                                "/placeholder.svg?height=50&width=50";
                            }}
...
function Purchase({ cart }) {
  ...
  return (
    <div className="border-t border-gray-200 pt-3 mt-1">
      ...
      <button
        type="button"
        className="w-full px-4 py-2.5 border border-black rounded-lg bg-black hover:bg-black/90 text-white transition-colors drop-shadow flex flex-row gap-x-2 items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2"
        onClick={() => createPayment(result)}
